...
\begin{homeworkProblem}[5]
  ...
  \begin{proof}
    Note that
    \[
      h_{n,k} = 2^{n/2}\lr{\ind_{[k2^{-n},(k+1/2)2^{-n})}-\ind_{[(k+1/2)2^{-n},(k+1)2^{-n}]}}
    \]
    Suppose that $f\in L^2([0,1])$ and $f\in \mc{E}^\perp$.
    For $n\in\N\cup\{0\}$, define a function $c_n:\{0,\ldots,2^{n+1}-1\}$ by $c_n(k) = \int_{[k2^{-(n+1)},(k+1)2^{-(n+1)}]}f(x)\dd{x}$. Then, for all $n\in\N\cup\{0\}$ and $0\leq k<2^n$,
    \[
      0 = \inp{f}{h_{nk}} \implies c_n(2k) = \int_{[k2^{-n},(k+1/2)2^{-n})}f(x)\dd{x} = \int_{[(k+1/2)2^{-n},(k+1)2^{-n}]}f(x)\dd{x} = c_n(2k+1).
    \]
    We show that $c_n(\cdot)$ is constant for all $n\in\N\cup\{0\}$ by induction on $n$. For $n=0$, we have already shown that $c_0(0) = c_0(1)$. Now suppose that $c_{n-1}(\cdot)$ is constant. Then, for $k\in\{0,\ldots,2^{-(n+1)}\}$
    \[
      c_{n-1}(0) = c_{n-1}(k) = c_n(2k)+c_n(2k+1) = 2c_n(2k) = 2c_n(2k+1),
    \]
    so $c_n(\cdot)$ is constant.\\

    However then Lebsegue differentiation theorem implies that $f$ is constant as we may always find a nicely shrinking sequence of sets in our aforementioned family, however the expression we would obtain for $f(x)$ would not depend on $x$ as $c_n$ is constant. Now the fact that $\int_0^1 f(x)\dd{x} = 0$ implies that $f=0$.
  \end{proof}
\end{homeworkProblem}
