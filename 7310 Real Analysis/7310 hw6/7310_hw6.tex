...
\begin{homeworkProblem}
  ...
  \begin{enumerate}
    [(a)]\item If $E\in \mc{M}\times\mc{N}$ and $\mu\times\nu(E) = 0$, then $\nu(E_x) = \mu(E^y) = 0$ for a.e. $x$ and $y$.
    \item If $f$ is Lebesgue measurable and $f=0$ Lebesgue almost everywhere, then $f_x$ and $f^y$ are integrable for a.e. $x$ and $y$, and $\int f_x \dd{\nu}$=0 and $\int f^y \dd{\mu}$ for a.e. $x$ and $y$.
  \end{enumerate}
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    \begin{align*}
      \int_{(0,a)}|g(x)|\dd{x} &= \int_{(0,a)}\left|\int_{(0,a)}t^{-1}f(t)\ind_E(x,t)\right|\dd{t}\dd{x} \leq \int_{(0,a)}\int_{(0,a)}| t^{-1}f(t)|\ind_{E^t}(x)\dd{x}\dd{t}\\ &= \int_{(0,a)} t^{-1}|f(t)| m(E^t)\dd{t} = \int_{(0,a)}|f(t)|\dd{t}<+\infty
    \end{align*}
