...
\begin{homeworkProblem}[5]
  ...
  \begin{proof}
    ...
    \]
    By the axiom of choice, $\ms{J}$ is countable. Let $\ms{B} = \{ B_t(x,d): (x,t)\in \ms{J}\}$. We claim that $\ms{B}$ is a basis for the metric topology on $(X,d)$. As $\ms{B}$ covers $D$ and $D$ is dense in $X$, it is clear that $\ms{B}$ covers $X$.\\
