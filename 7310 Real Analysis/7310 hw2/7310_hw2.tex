...
\begin{homeworkProblem}
  ...
  \begin{proof}\ \\
    \emph{(1$\implies$2)}: Let $(A_j)_{j=1}^{\infty}$ be a sequence in $\mc{A}$ such that $\bigcup_{j=1}^{\infty}A_j\in\mc{A}$. Define a new sequence $(A_n')_{n=1}^{\infty}$ by $A_1'=A_1$ and $A_n' = A_n \setminus \bigcup_{j=1}^{n-1}A_j$ for $n\geq 2$. Then by disjoint countable additivity and monotonicity,
    \[
    ...
    \emph{($3\implies 1$)}: Follows identically to the proof in problem 1 part (b) by assumingthe sequence is such that the countable union stays in $\mc{A}$.
  \end{proof}
\end{homeworkProblem}
...
\begin{homeworkProblem}[5]
  ...
  \begin{proof}
    ...
    \]
    whence $E_1\cup\cdots\cup E_n$ is a continuity set. Now suppose that $E\sub X$ is a continuity set. Then
    \[
  ...
  \begin{proof}
    ...
    \]
    where $1/0 := \infty$ by convention. Then $(0,r) = \bigcup_{n=1}^{\infty}A_n$, so there exists an $n\in \N$ such that $A_n$ is infinite. Take a countably infinite subset $\{ s_1, s_2, \ldots\}\sub A_n$. Then
    \[
