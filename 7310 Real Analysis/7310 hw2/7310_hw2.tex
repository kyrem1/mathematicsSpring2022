...
\begin{homeworkProblem}
  ...
  \begin{proof}\ \\
    ...
    \underline{$\impliedby$}: Let $(A_j)_{j=1}^{\infty}$ be a sequence of pairwise disjoint elements of $\mc{A}$ such that $A=\bigcup_{j=1}^{\infty}A_j\in\mc{A}$. Define a new sequence in $\mc{A}$ by $E_n = A\setminus\bigsqcup_{j=1}^{n}A_j$. Then $(E_n)_{n=1}^{\infty}$ is a decreasing sequence in $\mc{A}$ such that $\bigcap_{n=1}^{\infty}=0$. Hence, by assumption,
    \[
      0 = \lim_{n\to\infty}\mu(E_n) = \lim_{n\to\infty}\mu(A)-\mu\left(\bigsqcup_{j=1}^{n}A_j\right) = \mu(A)-\lim_{n\to\infty}\sum_{j=1}^{n}\mu(A_j) \implies \mu\left(\bigsqcup_{j=1}^{\infty}A_j\right) = \sum_{j=1}^{\infty}\mu(A_j),
    \]
    so $\mu$ is a premeasure.
  \end{proof}
\end{homeworkProblem}
...
\begin{homeworkProblem}[5]
  ...
  \begin{proof}
    We show the stronger statement that there are only countably many $s\in (0,r)$ such that $B_s(x,d)$ is not a continuity set. \\




    Suppose, for the sake of contradiction, that there exists uncountably many counterexamples $s\in (0,r)$ such that $\mu(\cls{B_s(x)}\setminus\Int(B_s(x)))\neq 0$. For $n\in \N$, define a set
    \[
      A_n=\{ s\in(0,r): \frac{1}{n}\leq \mu(\cls{B_s(x)}\setminus\Int(B_s(x))) < \frac{1}{n-1}\}
    \]
    where $1/0 := \infty$ by convention. Then, by assumption, $\bigcup_{n=1}^{\infty}A_n$ is uncountable, so there exists an $n\in \N$ such that $A_n$ is infinite. Take a countably infinite subset $\{ s_1, s_2, \ldots\}\sub A_n$. Note that, for any fixed $t\in(0,+\infty)$, $\cls{B_t(x)}\setminus\Int(B_t(x))\sub\{y\in X:d(x,y) = t\}$, whence the following union is disjoint:
    \[
  ...
  \begin{proof}
    Let $D\sub X$ be a countable dense subset of $X$. Fix $x\in D$. Then the set $\{ r\in(0,+\infty): \mu(B_{r_x}(x,d))<+\infty\}$ is nonempty, so define $R_x = \sup\{ r\in(0,+\infty): \mu(B_{r_x}(x,d))<+\infty\}\in [0, +\infty]$. By part (ii), we may take $J_x\sub (0, R_x)$ to be a countable, dense subset of $(0, R_x)$ such that $B_t(x,d)$ is a continuity set for all $t\in J_x$, whence $\inf_{t\in J_x}t = 0$

    % Define two sets $L_x, U_x\sub (0,r_x)$ as follows. For each $n\in \N$, by part (ii) there exists a $u_n\in (r_x-\frac{1}{n},r_x)$ and $l_n\in (0, \frac{r_x}{n})$ such that $B_{u_n}(x,d)$ and $B_{l_n}(x,d)$ are continuity sets. Let $L_x = \{ l_n : n\in \N\}$ and $U_x = \{ u_n : n\in \N\}$ and set $J_x = U_x \cup L_x$.

    \[
      \ms{J} = \bigsqcup_{x\in D} \{x\}\times J_x.
    \]
    By the axiom of choice, $\ms{J}$ is countable. Let $\ms{B} = \{ B_t(x,d): (x,t)\in \ms{J}\}$. We claim that $\ms{B}$ is a basis for the metric topology on $(X,d)$. Let $U\sub X$ be open and $p\in U$. Then there exist $r,r'>0$  such that $B_r(p,d)\sub U$ and $\mu(B_{r'}(p,d))<+\infty$. Take $\eps = \min\{ r,r'\}$, so $B_\eps(p,d)\sub U$ and $\mu(B_\eps(p,d))<+\infty$. By density, there exists an $x\in D$ such that $d(x,p)<\frac{\eps}{4}$. Note that $B_{\frac{3\eps}{4}}(x,d)\sub B_{\eps}(p,d)$, whence $\mu(B_{\frac{3\eps}{4}}(x,d))<+\infty$ so $\frac{2\eps}{4}<\frac{3\eps}{4}\leq R_x$. Then, by density of $J_x$ in $(0, R_x)$, there exists a $t\in J_x$ such that $|t-\frac{2\eps}{4}|<\frac{\eps}{100}$. Then $p\in B_t(x,d)$ and $B_t(x,d)$ is a continuity set in $U$.
