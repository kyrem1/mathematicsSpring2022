...
\begin{homeworkProblem}[5]
  ...
  \begin{proof}
    We show the stronger statement that, for all $\eps\in(0,r]$, there exists an $s\in (r-\eps,r)$ so that $B_s(x,d)$ is a continuity set. \\

    Suppose, for the sake of contradiction, that there exists an $\eps\in(0,r]$  such that $\mu(\cls{B_s(x)}\setminus\Int(B_s(x)))\neq 0$ for all $s\in(r-\eps,r)$. For $n\in \N$, define a set
    \[
      A_n=\{ s\in(r-\eps,r): \frac{1}{n}\leq \mu(\cls{B_s(x)}\setminus\Int(B_s(x))) < \frac{1}{n-1}\}
    \]
    where $1/0 := \infty$ by convention. Then, by assumption, $(r-\eps,r) = \bigcup_{n=1}^{\infty}A_n$ which is necessarily uncountable, so there exists an $n\in \N$ such that $A_n$ is infinite. Take a countably infinite subset $\{ s_1, s_2, \ldots\}\sub A_n$. Note that, for any fixed $t\in(0,+\infty)$, $\cls{B_t(x)}\setminus\Int(B_t(x))\sub\{y\in X:d(x,y) = t\}$, whence the following union is disjoint:
    \[
  ...
  \begin{proof}
    Let $D\sub X$ be a countable dense subset of $X$. Fix $x\in D$. Then there exists an $r_x>0$ such that $\mu(B_{r_x}(x,d))<+\infty$. Define two sets $L_x, U_x\sub (0,r_x)$ as follows. For each $n\in \N$, by part (ii) there exists a $u_n\in (r_x-\frac{1}{n},r_x)$ and $l_n\in (0, \frac{r_x}{n})$ such that $B_{u_n}(x,d)$ and $B_{l_n}(x,d)$ are continuity sets. Let $L_x = \{ l_n : n\in \N\}$ and $U_x = \{ u_n : n\in \N\}$ and set $J_x = U_x \cup L_x$. Then $J_x\sub(0,+\infty)$ is countable such that $\inf_{t\in J_x}t = 0$,  $\sup_{t\in J_x} = r_x$, and $B_t(x,d)$ is a continuity set for all $t\in J_x$. Let
    \[
      \ms{J} = \bigsqcup_{x\in D} \{x\}\times J_x.
    \]
    By the axiom of choice, $\ms{J}$ is countable. Let $\ms{B} = \{ B_t(x,d): (x,t)\in \ms{J}\}$. We claim that $\ms{B}$ is a basis for the metric topology on $(X,d)$. Let $U\sub X$ be open and $p\in U$. 

    % As $\ms{B}$ covers $D$ and $D$ is dense in $X$, it is clear that $\ms{B}$ covers $X$.\\
