...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    \[
      I_{(a_1,\ldots, a_k)} = \Set{ x\in [0,1]\setminus N: x=\sum_{n=1}^{k}a_k 2^{-k} + \sum_{j=k+1}^{\infty}b_j2^{-j} \text{ where }b_j\in\{0,1\}}
    \]
    Note that $\phi_k$ is constant on each $I_a$ for and is equal to $\sum_{n=1}^{k}a_n 2^{-n}$ for each $a\in \{ 0,1\}^{k}$, and each $x\in [0,1]\setminus N$ lies in some $I_k$, so
    \[
  ...
  \begin{proof}
    % TODO show trivial shit.
    $(a,b)\sub C^c$, so $\mu((a,b))\leq \mu(C^c) = 0$.
  \end{proof}
\end{homeworkProblem}
