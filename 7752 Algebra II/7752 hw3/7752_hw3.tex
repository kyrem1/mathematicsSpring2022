...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Define a map $\Phi:R^n\times R^m\to R^{nm}$ by $\Phi((r_1,\ldots, r_n), (r_1',\ldots, r_m')) = $
  \end{proof}
\end{homeworkProblem}
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    As $(e_1e_2)^2 =  (-e_2e_1)(e_1e_2) = -e_1^2 e_2^2 \in R_{<0}$, it follows that $e_1e_2\in V$. Observe that
    \begin{align*}
      B(e_1e_2, e_1) &= -\frac{e_1e_2e_1 + e_1e_1e_2}{2} = -\frac{e_1e_2e_1 - e_1e_2 e_1}{2} = 0\\
      B(e_1e_2, e_2) &= -\frac{e_1e_2e_2 + e_2e_1e_2}{2} = -\frac{-e_2 e_1 e_2 + e_2e_1e_2}{2} = 0
    \end{align*}
    Since $B$ is an inner product on $V$ and $e_1e_2$ is orthogonal to $\{e_1, e_2\}$ with respect to $B$, it follows that $e_1e_2$ is linearly independent from $\{ e_1,e_2\}$.
    As $1\not\in V$, $1$ is linearly independent from $\{ e_1,e_2,e_1e_2\}$.\\

    As $\{ 1,i,j\} \sub \mathbb{H}$ satisfy the relations for $\{ 1,e_1, e_2\}\sub A$, there exists an $\R$-algebra homomorphism $\phi:A\to \mathbb{H}$ such that $\phi(1)=1,\ \phi(e_1) = i,\ \phi(e_2) = j$. Observe that then $\phi(e_1 e_2) = \phi(e_1)\phi(e_2) = ij =k$, so $\phi$ is surjective by linearity. On the other hand, there exists an $\R$-algebra homomorphism $\psi:\mathbb{H}\to A$ such that $\psi(1) = 1,\ \psi(i)=e_1, \psi(j)=e_2$. The maps $\psi,\phi$ are clearly mutual inverses, so $A\cong \mathbb{H}$ as $\R$-algebras.
  \end{proof}

  \textbf{(e)} \textbf{Bonus:} Suppose $n\geq 2$. Prove that $D=H$. \textbf{Hint:} One way to proceed is to show that if $n>2$, then the multiplication  in $D$ cannot be associative.

  \begin{proof}
    Suppose, for the sake of contradiction, that $n>2$.
  \end{proof}
\end{homeworkProblem}
